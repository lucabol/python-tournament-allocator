...
"""
    ...
    def test_add_court_constraint(self, client, temp_data_dir):
        """Test adding a court-specific constraint."""
        import app as app_module
        from app import load_constraints
        ...
        # Add court constraint
        response = client.post('/constraints', data={
            'action': 'add_court_constraint',
            'court_name': 'Main Court',
            'available_after': '10:00',
            'available_before': '18:00',
            'court_note': 'Maintenance in morning'
        }, follow_redirects=True)
        
        assert response.status_code == 200
        
        constraints = load_constraints()
        assert 'court_specific_constraints' in constraints
        assert len(constraints['court_specific_constraints']) == 1
        assert constraints['court_specific_constraints'][0]['court_name'] == 'Main Court'
        assert constraints['court_specific_constraints'][0]['available_after'] == '10:00'
        assert constraints['court_specific_constraints'][0]['available_before'] == '18:00'
    
    def test_delete_court_constraint(self, client, temp_data_dir):
        """Test deleting a court-specific constraint."""
        import app as app_module
        from app import load_constraints, save_constraints
        ...
        constraints = {
            ...
            'court_specific_constraints': [
                {'court_name': 'Court 1', 'available_after': '10:00'}
            ]
        }
        ...
        # Delete constraint
        response = client.post('/constraints', data={
            'action': 'delete_court_constraint',
            'court_name': 'Court 1'
        }, follow_redirects=True)
        
        assert response.status_code == 200
        
        updated_constraints = load_constraints()
        assert len(updated_constraints.get('court_specific_constraints', [])) == 0
    
    def test_court_constraint_updates_existing(self, client, temp_data_dir):
        """Test that adding constraint for same court replaces existing."""
        import app as app_module
        from app import load_constraints
        ...
        # Add another constraint for same court (should replace)
        client.post('/constraints', data={
            'action': 'add_court_constraint',
            'court_name': 'Court X',
            'available_after': '11:00'
        })
        
        constraints = load_constraints()
        court_constraints = [c for c in constraints.get('court_specific_constraints', []) 
                           if c['court_name'] == 'Court X']
        
        assert len(court_constraints) == 1
        assert court_constraints[0]['available_after'] == '11:00'
